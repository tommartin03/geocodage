...
def convert(chemin_excel):
    ...
    with open("Données.csv", "w", newline='', encoding='ISO-8859-1') as fichier_csv:
        ...
        for ligne in active_workbook.iter_rows():
            ...
            ecrire.writerow([str(cell.value) for cell in ligne])
...
def filtrer():
    ...
    with open("Resultats.csv", "w", newline='', encoding='ISO-8859-1') as fichier_csv:
        writer = csv.writer(fichier_csv, delimiter=',',quotechar ='"',quoting=csv.QUOTE_ALL)
        #ouvre le fichier sans filtre
        with open("Ban.csv", newline='', encoding='ISO-8859-1') as donnees_csv:
            #lit le fichier 
